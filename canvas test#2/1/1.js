	...
	var canvasBody = document.getElementById("canvas"),
		...
		Particle = function(Xpos, Ypos) {
			...
			this.d = {
				//Object, stores directions. Computes directions according to the random this.directionAngle
				x: Math.cos(this.directionAngle) * this.speed,
				y: Math.sin(this.directionAngle) * this.speed
			};
			this.update = function() {
				//The update function. The function that calculates next coordinates
			...
			this.border = function() {
				//The border function. Checks if this thing touches the border
			...
			this.draw = function() {
				//Just draws the points. Pretty easy. Takes the coords, color, radius - draws.
				canvas.beginPath();
		...
		checkDistance = function(x1, y1, x2, y2) {
			//You got it. The point on the graph distance formula.
	...
	function setup() {
		//Function called once to set everything up
	...
	function loop() {
		//Function of loop that will be called for a frame of the animation
		window.requestAnimationFrame(loop);
		tick++;
